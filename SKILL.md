...
8. 桌面和移动端都必须检查：无页面横向滚动，mini mockup 占满或居中，内部文字/控件不互相压住
9. 四方向预览桌面默认 2×2 左右两栏，移动端单列；除非全宽沉浸带有明确必要
10. 预览页提供三拨盘滑块、选择确认弹层和调整建议输入框；滑块拖动时数值必须即时变化，确认后回传方向 + 拨盘 + 建议
11. 预览页没有 60 秒自动推进或自动确认；必须用户点选/键盘选择并确认，或在对话中明确选择
12. 最终回复必须提供预览目录、`index.html` 的可点击本地路径和本地预览 URL（若已启动）
13. 交付前必须运行最低限度检查：文件存在、HTML 可解析、包含 4 个方向名、包含「推荐」
